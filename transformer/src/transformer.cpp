...
// Original SelfAttentionHead with causal masking support
SelfAttentionHead::SelfAttentionHead(int context_len, int embedding_dim, int parameter_dim, bool is_causal)
        : context_len(context_len),
          ...
          V(torch::nn::Linear(embedding_dim, parameter_dim))  // Change this to parameter_dim
{
    register_module("Q", Q);
    register_module("K", K);
    register_module("V", V);
...
torch::Tensor FeedForward::forward(torch::Tensor x) {
    x = fc1->forward(x);
    x = torch::gelu(x);  // GELU activation
    x = dropout->forward(x);
    x = fc2->forward(x);
    x = dropout->forward(x);
    return x;
...
// Original Encoder (kept for compatibility)
...
// Original EncoderDecoderAttentionHead (kept for compatibility)
...
// Original Decoder (kept for compatibility)
...
// Original Transformer (kept for compatibility)
